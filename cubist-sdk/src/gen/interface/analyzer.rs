...
pub struct Analyzer {
    ...
    interface_targets: Map<ContractFile, Set<Target>>,
...
impl Analyzer {
    ...
    pub fn get_target_info(&self) -> &Map<ContractFile, Set<Target>> {
        &self.interface_targets
    }

    /// Determine which functions reside in which files
    ...
    pub fn analyze(&mut self, sources: &[SourceFile]) -> Result<()> {
        ...
        for source in sources.iter() {
            self.visit_source_unit(&source.pt)?;
        }
        Ok(())
    }
            ...
            if let pt::ContractPart::VariableDefinition(def) = cp {
                ...
                if let Some(contract_name) = maybe_contract_name {
                    ...
                    if current_target != object_target {
                        ...
                        self.add_interface_target(contract_file, current_target)?;
                    }
                }
            }
        }
    ...
    fn id_cross_target_calls(&mut self, expr: &pt::Expression) -> Result<()> {
        if let pt::Expression::FunctionCall(_, fun, _) = expr {
            if let pt::Expression::MemberAccess(_, base, call) = &**fun {
    ...
    fn add_interface_target(&mut self, source: ContractFile, target: Target) -> Result<()> {
        self.interface_targets
            .entry(source)
            .or_default()
            .insert(target);
