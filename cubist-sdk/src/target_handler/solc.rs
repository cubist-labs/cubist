...
fn parse_contract(json_path: &Path) -> Result<(Abi, Bytes)> {
    let invalid_contract = |reason: &str, source: Option<WrapperError>| {
    ...
    let json = fs::read_to_string(json_path)
        ...
        .map_err(|e| invalid_contract("Read error", Some(e)))?;
    ...
    let val: serde_json::Value = serde_json::from_str(json.as_str())
        ...
        .map_err(|e| invalid_contract("Malformed JSON", Some(e)))?;
    ...
    let abi: Abi = serde_json::from_value(abi.to_owned())
        ...
        .map_err(|e| invalid_contract("Invalid 'abi' value", Some(e)))?;
    ...
    let bytes = Bytes::from_str(bytes)
        ...
        .map_err(|e| invalid_contract("Property 'bytecode.object' is not a hex string", Some(e)))?;
...
impl ContractCompiler for SolcCompiler {
        ...
        for dir in [
            ...
            if dir.is_dir() {
                fs::remove_dir_all(dir)
                    ...
                    .map_err(|e| CubistSdkError::CleanError(dir.clone(), e))?
            }
        }
        Ok(())
    }
    ...
    fn find_compiled_contracts(&self, source_file: &Path) -> Result<HashMap<String, ContractInfo>> {
        ...
        let read_dir_result = std::fs::read_dir(&artifacts_dir)
            ...
            .map_err(|e| CubistSdkError::NoArtifactsDir(artifacts_dir, e))?;
