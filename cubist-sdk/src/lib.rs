...
use ethers::types::{ParseBytesError, H256};
...
pub enum WrapperError {
    ...
    #[error("Error sending transaction {0:?} to {1}: {2}")]
    SendTransactionError(String, Target, String),
    #[error("Could not fetch transaction with hash '{0}' @ {1}. Reason: {2}")]
    TransactionNotFound(H256, Target, String),
}
...
pub enum CubistSdkError {
    ...
    ParseContractError(PathBuf, String, #[source] Option<Box<WrapperError>>),
    /// Error raised when artifacts directory cannot be read.
    #[error("Failed to read artifacts directory {0}.\nDid you run 'cubist build'?")]
    NoArtifactsDir(PathBuf, #[source] Box<WrapperError>),
    /// Error raised when loading bridge file fails
    #[error("Failed to load bridge for contract {0} (from bridge file {1})")]
    LoadBridgeError(ContractFQN, PathBuf, #[source] Box<WrapperError>),
    /// Error raised when a compilation fails.
    #[error("Error compiling {0}: {1}")]
    CompileError(PathBuf, String, #[source] Option<Box<WrapperError>>),
    /// Error raised when cleaning build directory fails
    #[error("Error cleaning project build directory {0}")]
    CleanError(PathBuf, #[source] Box<WrapperError>),
    /// Error raised when creating a deployer fails.
    #[error("Failed to connect to target '{0}' at {1}")]
    CreateClientError(Target, SecretUrl, #[source] Option<Box<WrapperError>>),
    /// Error raised when deployment fails.
    #[error("Error deploying contract '{0}' to {1}")]
    DeployError(ContractFQN, Target, #[source] Box<WrapperError>),
    /// Error raised when Soroban deployment fails.
    #[error("Error deploying contract '{0}' through Soroban: {1}")]
    SorobanDeployError(ContractFQN, String),
    ...
    /// Error raised when updating an Axelar receiver shim contract's target fails.
    #[error("Failed to update '{receiver_contract}' on chain '{target}'")]
    AxelarSetTargetError {
        /// Receiver shim contract whose target needed to be set
        receiver_contract: ContractFQN,
        /// Target chain
        target: Target,
    },
    ...
    SaveDeploymentManifestError(PathBuf, #[source] Box<WrapperError>),
    /// Error raised when saving deployment receipt fails
    #[error("Failed to save deployment receipt to file {0}")]
    SaveDeploymentReceiptError(PathBuf, #[source] Box<WrapperError>),
    /// Error raised when saving deployment receipt fails
    #[error("Failed to deserialize deployment receipt from file {0}")]
    DeserializeDeploymentReceiptError(PathBuf, #[source] Box<WrapperError>),
    ...
    /// Error raised when retrieving balance from the chain provider fails
    #[error("Failed to retrieve balance of account '{0}' on chain '{1}': {2}")]
    GetBalanceError(String, Target, String),
    /// Error raised when retrieving particular chain provider fails
    #[error("Failed to retrieve project for chain '{0}'")]
    ProjectError(Target),
    ...
    /// Error raised loading Axelar manifest from disk fails.
    #[error("Could not load Axelar manifest. Did you start the relayer?")]
    LoadAxelarManifest(#[source] WrapperError),
