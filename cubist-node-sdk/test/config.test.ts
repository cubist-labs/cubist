...
import { URL, } from 'url';
import { BridgeProvider, } from '../dist';
...
describe('Config.from_file', () => {
  describe('good config', () => {
    afterEach(() => {
      delete process.env.CUBIST_BUILD_DIR;
      delete process.env.CUBIST_DEPLOY_DIR;
      delete process.env.CUBIST_NETWORK_PROFILE;
      delete process.env.CUBIST_BRIDGE_PROVIDER;
    });
    it('apply overrides', () => {
      process.env.CUBIST_BUILD_DIR = 'b1';
      process.env.CUBIST_DEPLOY_DIR = 'd1';
      process.env.CUBIST_BRIDGE_PROVIDER = 'Axelar';
      process.env.CUBIST_NETWORK_PROFILE = 'testnets';
      ...
      expect(cfg.build_dir()).to.equal(path.normalize(path.join(dir, 'b1')));
      expect(cfg.deploy_dir()).to.equal(path.normalize(path.join(dir, 'd1')));
      expect(cfg.bridge_provider).to.equal(BridgeProvider.Axelar);
      expect(cfg.current_network_profile).to.equal('testnets');
    });
    it('create config', () => {
      const dir = path.join(__dirname, 'fixtures', 'config-fixtures');
      const cfg = Config.from_file(
      ...
      expect(network_profile.avalanche?.url.toString()).to.equal('http://otherhost:9560/');
      expect(network_profile.avalanche?.num_nodes).to.equal(5);
      expect(network_profile.avalanche?.autostart).to.equal(false);
      expect(network_profile.avalanche?.subnets.length).to.equal(0);
