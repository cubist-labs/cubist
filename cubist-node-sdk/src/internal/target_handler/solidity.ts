...
class ContractFactory implements ProjectFactory {
...
export class Handler extends TargetProjectHandler {
  ...
  /** Underlying signer account. */
  private _defaultSignerAccount: AccountAddressOrIndex;
  ...
  constructor(project: TargetProject) {
    ...
    // The default signer account is the first account.
    this._defaultSignerAccount = 0;
  }
  ...
  * */
   ...
   * Uses {@link getDefaultSignerAccount | default account} when called with no argument.
   * @param {AccountAddressOrIndex?} addrOrIndex - Optional account address or index.
   * @return {Signer} The signer. */
  getSigner(addrOrIndex?: AccountAddressOrIndex): Signer {
    const account = addrOrIndex ?? this._defaultSignerAccount;
    return this.provider.getSigner(account);
  }

  /** Retrieve all managed accounts used on this target.
   ...
   */
  async getDefaultSignerAccount(): Promise<AccountAddress> {
    if (typeof this._defaultSignerAccount === 'number') {
      return (await this.accounts())[this._defaultSignerAccount as number];
    } else {
      return this._defaultSignerAccount;
    }
  }
   ...
   * @return {Promise<void>} promise to set the signer.
   */
  setDefaultSignerAccount(addrOrIndex: AccountAddressOrIndex): Promise<void> {
    return new Promise(() => {
      this._defaultSignerAccount = addrOrIndex;
    });
  }
