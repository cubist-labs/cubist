 ...
 * @module
 */
import { Config, PathBuf, } from './config';
import { Cubist, } from './cubist';
import * as path from 'path';
import { emptyDir, copySync, readFileSync, writeFileSync, } from 'fs-extra';
...
export class TestDK<T extends Cubist> {
  ...
  * Configs too). */
  ...
  /**
   * Copy the entire project folder to a fresh temp folder.
   *
   * @param {PathBuf} cfg - Original config file whose parent folder to copy.
   * @param {[string, string][]} remap - Optional string->string remapping
   *   to apply to the contents of the config file before writing it to the new location.
   * @return {PathBuf} Full path to the relocated config file
   */
  static copyToTmpDir(cfg: PathBuf, remap?: [string, string][]): PathBuf {
    const srcDir = path.dirname(cfg);
    const cfgName = path.basename(cfg);
    const tmp = mkdtempSync(path.join(tmpdir(), 'cubist-node-sdk-test-'));
    copySync(srcDir, tmp, { overwrite: true, });
    const newCfgPath = path.join(tmp, cfgName);
    if (remap) {
      const fileContents = readFileSync(newCfgPath, 'utf8');
      const remappedContents = remap.reduce(
        (acc, t) => acc.replaceAll(t[0], t[1]),
        fileContents
      );
      writeFileSync(newCfgPath, remappedContents);
    }
    return newCfgPath;
  }
