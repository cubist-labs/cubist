...
export interface TestDKOptions {
  ...
  /** Arguments to call {@link Cubist} (or Cubist-like) constructor with. */
 ...
 * create a new project, so anything that extends {@link Cubist} is sufficient.
 *
 * @group Testing
 * */
...
/**
 * Class that abstracts over a Cubist project's testing process. In particular, this
 * class makes it possible to (1) create {@link Cubist} (or `CubistORM`) projects
 * with temporary build and deploy directories and (2) start and stop {@link
 * Service | services} when running tests.
 *
 * @example
 *
 * Using {@link TestDK} to test `CubistORM` projects with [Jest](https://jestjs.io/).
 * The `cubist build` command generates `CubistORM` at build time; this class
 * extends {@link Cubist} with project-specific contracts (see [Overview](/jsdoc/)).
 *
 * ```typescript
...
export class TestDK<T extends Cubist> {
  ...
  * Configs too). */
   ...
   *
   * **NOTE**: Whenever you use {@link TestDK}, you should always use this
   * accessors to get a reference to the {@link Cubist} project. Creating a new
   * {@link Cubist} object could have unwanted behavior. Here is an example how
   * things can go wrong:
   *
   * ```
   * const testdk = new TestDK<CubistORM>(CubistORM, { tmp_deploy_dir: true });
...
/**
 * Wrapper for {@link TestDK} specific to {@link Cubist}, eliding the need for
 * (1) a type parameter and (2) passing {@link Cubist} as an argument.
 *
 * @example
 * Testing already-built (and deployed) project.
 * ```typescript
