...
export enum BridgeProvider {
  /** Use Cubist bridging relayers */
  Cubist = 'Cubist', // eslint-disable-line no-unused-vars
  /** Use Axelar's interfaces and relayers */
  Axelar = 'Axelar', // eslint-disable-line no-unused-vars
}
...
export class Config {
  ...
  * profile with the same name must be defined in `network_profiles`. */
  ...
  /** The bridge provider to use for cross-chain invocations. */
  get bridge_provider(): BridgeProvider {
    return this._bridge_provider;
  }
  private _bridge_provider: BridgeProvider;
  ...
  **/
  ...
  static _from_json(json: IConfig, config_path: PathBuf): Config {
    ...
    self._bridge_provider = self.json.bridge_provider;
    ...
    ifenv('CUBIST_BUILD_DIR', (bd) => {
      self.__set_build_dir(bd);
    });
    ifenv('CUBIST_DEPLOY_DIR', (dd) => {
      self.__set_deploy_dir(dd);
    });
    ifenv('CUBIST_NETWORK_PROFILE', (np) => {
      self._current_network_profile = np;
    });
    ifenv('CUBIST_BRIDGE_PROVIDER', (bp) => {
      self._bridge_provider = bp === 'Axelar' ? BridgeProvider.Axelar : BridgeProvider.Cubist;
    });
    ...
    // Set resolver preference to ipv4
