...
impl CredProxy {
    ...
    pub fn from_cfg(cfg: &EthProxyConfig) -> Result<Self, FatalErr> {
        ...
        tracing::info!("Creating credentials proxy for {:?}", accounts.addresses());
    ...
    async fn nonce_for_account(&self, a: Address) -> Result<U256, ProviderError> {
        // If we don't have a provider, just return 0.  This is useful in testing
        if self.provider.is_none() {
            return Ok(0.into());
        }

        // Let's check the transaction count on the remote end.
        let provider = self.provider.as_ref().unwrap();
        let trx_count = provider
            .get_transaction_count(a, Some(BlockNumber::Pending.into()))
            .await?;
        tracing::debug!("Transaction count for account {a} is {trx_count}");
        Ok(trx_count)
    }
    ...
    async fn sign_transaction(
        ...
        match transaction.nonce() {
            ...
            // The nonce was provided for us, nothing we need to do
            Some(_) => (),
        }
