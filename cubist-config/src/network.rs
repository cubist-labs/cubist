...
pub struct NetworkProfile {
    ...
    /// configuration for a Stellar endpoint
    #[serde(skip_serializing_if = "Option::is_none")]
...
impl NetworkProfile {
    ...
    pub fn get(&self, name: Target) -> Option<EndpointConfig> {
        match name {
            ...
            Target::Stellar => self.stellar.clone().map(EndpointConfig::Stellar),
        }
    }

    /// Returns all defined endpoints
    pub fn endpoints(&self) -> impl Iterator<Item = EndpointConfig> + '_ {
        // TODO: derive all variants from enum definition
        let targets = [
            Target::Ethereum,
            Target::Avalanche,
            Target::AvaSubnet,
        ...
        targets.into_iter().flat_map(|t| self.get(t))
    }
}
...
/// A config for Stellar endpoints
#[derive(Clone, Debug, Deserialize, Serialize, JsonSchema)]
#[serde(deny_unknown_fields)]
