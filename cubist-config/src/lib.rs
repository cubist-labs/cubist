...
//! - [`bridge_provider`](Config::bridge_provider): currently selected bridge provider (can be overridden via `CUBIST_BRIDGE_PROVIDER` env var).
//!
//! Example JSON file:
//! ```
...
/// Manifest produced by the 'cubist relayer' step (when configured to use Axelar)
pub mod axelar_manifest;
/// Bridge metadata
pub mod bridge;
/// Well-known paths
pub mod paths;
...
#[derive(
    Default, Clone, Copy, PartialEq, Eq, FromStr, Deserialize, Serialize, JsonSchema, Debug,
)]
#[serde(deny_unknown_fields)]
pub enum Compiler {
    ...
    #[serde(rename = "solang")]
    Solang,
    /// Compile with the Soroban compiler.
...
pub enum TargetConfig {
    /// Target configuration for an EVM target
    EvmTargetConfig {
    ...
    /// Target configuration for Stellar
    StellarTargetConfig {
        /// The root of the Stellar workspace
        root: PathBuf,
        /// The contracts in the workspace
...
impl TargetConfig {
    ...
    pub fn source_files(&self) -> Vec<PathBuf> {
        match &self {
            Self::EvmTargetConfig { files, .. } => match &files {
                GlobsOrPaths::Paths(files) => files.clone(),
                _ => {
                    panic!("BUG: TargetConfig::files() called before TargetConfig::resolve_globs()")
                }
            },
            Self::StellarTargetConfig { root, .. } => {
                // List the compiled Soroban files
                glob(&format!("{}/src/**/*.rs", root.display()))
                    .unwrap()
                    .filter_map(|entry| match entry {
                        Ok(path) => Some(path.clone()),
                        Err(e) => {
                            warn!("Error while trying to collect Soroban files: {e}");
                            None
                        }
                    })
                    .collect()
    ...
    /// Get the list of files containing contracts for this target.
    pub fn contract_files(&self) -> Vec<PathBuf> {
        match &self {
            Self::EvmTargetConfig { .. } => self.source_files(),
            Self::StellarTargetConfig { root, contracts } => {
                // `soroban contract build` performs a release build with the
                // `wasm32-unknown-unknown` target
                // (<https://soroban.stellar.org/docs/getting-started/hello-world#build>)
                contracts
                    .iter()
                    .map(|contract| {
                        root.join("target")
    ...
    pub(crate) fn resolve_paths(&mut self, cfg: &Config) -> Result<()> {
        match self {
            Self::EvmTargetConfig { files, .. } => {
                if let GlobsOrPaths::Globs(globs) = files {
                    *files = GlobsOrPaths::Paths(Self::resolve_globs_pure(globs, cfg)?);
                }
            }
            Self::StellarTargetConfig { root, .. } => {
                // Resolve root path of Stellar workspace
                *root = cfg.absolute_path_in_project(root.clone());
            }
        }
        Ok(())
    }

    fn resolve_globs_pure(globs: &[Glob], cfg: &Config) -> Result<Vec<PathBuf>> {
...
pub struct ContractsConfig {
    ...
    pub solidity_dependencies: HashMap<String, String>,
}
...
impl Default for ContractsConfig {
    fn default() -> Self {
        ContractsConfig {
            ...
            solidity_dependencies: HashMap::new(),
        }
    }
}
...
impl CompilerConfig {
    /// Search for `path` in this config's import dirs ([`Self::import_dirs`]).
    pub fn search<P: AsRef<Path>>(&self, path: P) -> Option<PathBuf> {
        self.import_dirs.iter().find_map(|dir| {
...
impl Config {
    ...
    /// * Set `bridge_provider` via `CUBIST_BRIDGE_PROVIDER`
    ///
    ...
    fn merge_paths_from_env(&mut self) {
        ...
        if let Ok(bp) = env::var("CUBIST_BRIDGE_PROVIDER") {
            if let Ok(bp) = serde_json::from_value::<BridgeProvider>(json!(bp.as_str())) {
                tracing::debug!("Setting bridge provider from CUBIST_BRIDGE_PROVIDER to {bp:?}");
                self.bridge_provider = bp;
            } else {
                tracing::warn!("Invalid CUBIST_BRIDGE_PROVIDER: '{bp}'");
            }
        }
    }
    ...
    fn validate_contract_paths(&self) -> Result<()> {
        ...
        for (target, t_config) in contracts.targets.iter() {
            let files = t_config.source_files();
            if files.is_empty() {
