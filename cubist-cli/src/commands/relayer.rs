...
use console::style;
use cubist_sdk::{
    core::{Contract, Cubist, DeploymentManifest},
    Http,
};
use ethers_contract::EthLogDecode;
use ethers_core::abi::{Error, RawLog, Token};
...
async fn resolve_contracts<M: Middleware>(
    ...
    let target_contract = cubist
        .find_contract(target, false, &manifest.contract)
        .ok_or_else(|| {
            eyre!(
    ...
    for shim in &manifest.shims {
        let shim_contract = cubist
            .find_contract(shim.target, true, &manifest.contract)
            .ok_or_else(|| {
                eyre!(
                    "Shim '{}' not found for '{}'",
                    &manifest.contract,
        ...
        shim_contract.at(shim.address).await?;
...
impl<M: Middleware + 'static> RelayerInner<M> {
    ...
    async fn relay_events(
        ...
        let ev = ethers_contract.event_for_name::<EvDe>(&ev_name)?;
    ...
    async fn forward_event(
        ...
        let args = from
            .inner()?
            .decode_event_raw(ev_name, log.topics, log.data.into())?;
