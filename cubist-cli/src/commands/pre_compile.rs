...
impl PreCompiler {
    pub fn new(config: &Config) -> Result<Self> {
        ...
        Ok(PreCompiler {
            paths,
            contracts,
            bridge_provider: config.bridge_provider.clone(),
            interfaces,
        })
    }
    ...
    fn fetch_external_imports(
        ...
        let backend = <dyn Backend>::create(&config.bridge_provider);
        js_pkg_imps.extend(backend.npm_dependencies());
    ...
    fn generate_all(&self) -> Result<()> {
        let backend = <dyn Backend>::create(&self.bridge_provider);
