...
use cubist_sdk::core::{ContractData, ContractInfo, CubistInfo};
...
/// Create TOML [`Table`] instance corresponding to the project Cargo.toml.
/// We currently choose defaults (e.g., for version and edition), but in the future may ask for user
/// input.
pub fn cargo_toml(name: &str) -> Table {
    let cubist_git_url = "ssh://git@github.com/cubist-labs/cubist.git";
    toml! {
        [package]
        name = name
        version = "0.1.0"
        ...
        for (t, cs) in &cubist.contracts {
            for c in cs {
                match &c.data {
                    ...
                    ContractData::SorobanData {
                        wasm_path,
                        spec_entries,
                        hash,
                    } => {
                        let code = soroban_spec_rust::generate(
                            spec_entries,
                        ...
                        let file = contracts_dir.join(module_name.clone()).with_extension("rs");
                        log_generated(&file);
                        let mut bindings_file = fs::File::create(file)?;
                        bindings_file.write_all(code.to_formatted_string().unwrap().as_bytes())?;
                        mods.push((module_name, c, t));
                    }
                }
            }
        }
