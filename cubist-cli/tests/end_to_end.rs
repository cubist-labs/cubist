...
async fn do_circular_foo_bar<M: Middleware + 'static>(proj: Cubist<M>) -> Result<()> {
    ...
    foo.deploy(bar.address_on(foo.target())).await?;
    bar.deploy(foo.address_on(bar.target())).await?;
...
async fn counter_testnets() -> Result<()> {
    ...
    println!("Deploying contracts");
    to.deploy(()).await?;
    from.deploy(to.address_on(from.target())).await?;
...
async fn mpmc() -> Result<()> {
    ...
    r1.deploy(()).await?;
    r2.deploy(()).await?;
    ch.deploy((r1.address_on(ch.target()), r2.address_on(ch.target())))
        .await?;
    s1.deploy(ch.address_on(s1.target())).await?;
    s2.deploy(ch.address_on(s2.target())).await?;
...
fn update_cargo_toml(app_dir: &Path) -> Result<()> {
    ...
    dependencies.insert(String::from("cubist-sdk"), toml! { path = cubist_sdk_path });
    dependencies.insert(
        String::from("cubist-config"),
        toml! { path = cubist_config_path },
    );
...
async fn deploy_and_test_api_mid(cfg: Config) -> Result<()> {
    ...
    sender
        .deploy((val, receiver.address_on(sender.target())))
        .await?;
...
fn assert_receipt(proj: &TargetProject, file: &str, name: &str, addr: Address) {
    let path = proj
        .target_paths
        .deploy_root
        .join(file)
        .join(name)
        .join(paths::hex(addr.as_fixed_bytes()))
        .with_extension("json");
...
fn assert_deployment_manifest(cnt: &Contract) -> Result<()> {
    ...
    let manifest_path = cnt
        .project
        .paths
        .for_deployment_manifest(&cnt.meta.fqn, cnt.address().unwrap().as_fixed_bytes());
    assert!(
    ...
    let check_deployment_info = |c: &Contract, d: &DeploymentInfo| {
        assert_eq!(c.address(), Some(d.address));
